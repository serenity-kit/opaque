...
// The changelogs are generated by `changeset version` and committed, but main()
// wipes the whole build folder. Read them before that happens so they can be
// restored afterwards. They don't exist yet on a fresh build, hence the guard.
const readChangelog = function (buildName) {
  const file = path.join(__dirname, "..", "build", buildName, "CHANGELOG.md");
  return fs.existsSync(file) ? fs.readFileSync(file, "utf8") : null;
};

const changelogs = {
  ristretto: readChangelog("ristretto"),
  p256: readChangelog("p256"),
};
...
function main() {
  ...
  // restore the changelogs removed by the `rm -rf build` above
  Object.entries(changelogs).forEach(([buildName, changelog]) => {
    if (changelog != null) {
      fs.writeFileSync(`build/${buildName}/CHANGELOG.md`, changelog);
    }
  });
